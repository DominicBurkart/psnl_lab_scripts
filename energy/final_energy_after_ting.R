...
plotmeans(short$`sum(kwh).x` ~ short$month, ylab = "Municipal Power 2017", xlab="Month") 
plotmeans(short$`sum(kwh).y` ~ short$month, ylab = "Municipal Power 2016", xlab="Month")
#plotmeans(short$`sum(kwh).x` ~ interaction(short$month, short$MunicipalCityCode), ylab="Municipal Power 2017", xlab="Muni x Month interaction") #not helpful
plot(short$`sum( customers).x`, short$`sum(kwh).x`, xlab="Customers 2017", ylab="Power Usage 2017") #generally conic; good.
plotmeans(short$`sum(kwh).x` / short$`sum( customers).x` ~ interaction(short$month, short$MunicipalCityCode), connect = F, xlab="Muni X Month", ylab="Ratio of power to customers")
plotmeans(short$`sum(kwh).x` / short$`sum( customers).x` ~ short$month, connect = F, xlab="Month 2017", ylab="Ratio of power to customers")


power_users_2017 = subset(short, short$`sum(kwh).x` / short$`sum( customers).x` > 10000)
power_users_2016 = subset(short, short$`sum(kwh).y` / short$`sum( customers).y` > 10000)
plotmeans(power_users_2016$`sum(kwh).x` ~ power_users_2016$month, xlab="month", ylab="teterboro usage 2017")
plotmeans(power_users_2016$`sum(kwh).y` ~ power_users_2016$month, xlab="month", ylab="teterboro usage 2016")


for (muni in unique(short$MunicipalCityCode)){
  par(new=TRUE)
  d = subset(short, MunicipalCityCode == muni)
  plotmeans(d$`sum(kwh).y` ~ d$month, ylim=c(0, 31454287))
}
...
r = lmer(`sum(kwh).x` ~  condition_imputed.x + scale(`weighted_thi.x`)  + (1 |  MunicipalCityCode), data=short, weights = cust_weights)
summary(r)
...
r = lmer(log(`sum(kwh).x`) ~  threemonths * Condition.x + scale(`weighted_thi.x`)  + (1 |  MunicipalCityCode), 
         data=subset(short, `sum(kwh).x` > 1e5), weights = cust_weights)
summary(r)
