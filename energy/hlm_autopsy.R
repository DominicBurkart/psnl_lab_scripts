...
munidf = munidf[order(munidf$condition_imputed),]
...
r = lmer(kwh ~ condition + ( thi | city) + (1 | month), weights = cust_weights)
summary(r)
plot(resid(r), col = condition, main="Residuals for non-interaction with random slope THI")
plot(r)
plotmeans(resid(r) ~ condition, data = munidf, main="Residuals for non-interaction with random slope THI")


condition = relevel(condition, "Co")
r = lmer(kwh ~ condition + thi + ( 1 | city) + (1 | month), weights = cust_weights)
summary(r)
plot(resid(r), col = condition, main="Residuals for non-interaction with covariate THI")
plot(r)
plotmeans(resid(r) ~ condition, data = munidf, main="Residuals for non-interaction with covariate THI")


condition = relevel(condition, "Tr")
r = lmer(kwh ~ condition + ( thi | city) + (1 | month))
summary(r)
