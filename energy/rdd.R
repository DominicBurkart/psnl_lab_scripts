...
summary(lm(avg.power.use ~ MunicipalCityCode + Condition, data=df2)) #Municipality / Treatments are too colinear without collapsing by town
...
df3 = sqldf(' select sum("avg.power.use"),sum("cdd"),"Condition","MunicipalCityCode" from df2 group by "MunicipalCityCode" ')
colnames(df3) = c("power", "cdd", "condition", "city")
summary(aov(power ~ condition, data = df3)) #F of 0.285 P of 0.887
